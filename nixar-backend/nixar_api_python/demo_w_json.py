...
from test_utils import create_schema_if_not_exist, create_credential_definition, \
  get_timestamp_tag, get_presentation_request, generate_sample_cred_values, create_nixar_agent_w_json_wallet


def demo_revocable():
  schema_name = "PythonSchemaRevocableV.0.1"
...
def demo_non_revocable():
  schema_name = "PythonSchemaNonRevocableV.0.1"
  attribute_set = ["name", "surname", "age", "gender"]
  schema_id = create_schema_if_not_exist(issuer, schema_name, attribute_set)
...
if __name__ == '__main__':
  ...
  issuer = create_nixar_agent_w_json_wallet("python_issuer", lambda: issuer_password, "ENDORSER")
  logging.info("Thw issuer agent was created")

  prover_password = test_utils.native_string("123456")
  prover = create_nixar_agent_w_json_wallet("python_prover", lambda: prover_password, None)
  logging.info("The holder agent was created")

  verifier_password = test_utils.native_string("123456")
  verifier = create_nixar_agent_w_json_wallet("python_verifier", lambda: verifier_password, None)
